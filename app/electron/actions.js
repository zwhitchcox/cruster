...
module.exports.actionsListen = ({mainWindow, settings}) => {
    ...
    try {
      switch (msg.type) {
        ...
        case "end-ssh-term":
          ...
          break
        case "run-background":
          status = await runBackground(msg)
          break
        case "scp-download":
          status = await scpDownload(msg)
          break
        default:
  ...
  const addPublicKeysGithub = async ({ghUsername, overwrite, enableReset}) => {
    let keys
    try {
  ...
  const runBackground = ({cmd, username}) => {
    username = typeof username === "undefined" ? "root" : username
    return new Promise(async (res, rej) => {
      const privateKey = (await fs.readFile(settings.privateKeyFile)).toString()
      const conn = new Client();
      conn.on('ready', () => {
        conn.exec(cmd, (err, stream) => {
          if (/^reboot/.test(cmd) && username === "root") {
            res("Rebooting...")
          }
  ...
  const scpDownload = ({ip, remotePath, localPath}) => {
    return new Promise(async (res, rej) => {
      const privateKey = (await fs.readFile(settings.privateKeyFile)).toString()
      ...
      conn.on('ready', () => {
        conn.sftp((err, sftp) => {
          if (err) {
            return rej(`couldn't connect to ${ip}`)
          }
          sftp.fastGet(remotePath, localPath, (err) => {
            if (err) rej(err)
            res(`Downloaded ${remotePath} successfully.`)
          })
        })
      })
